...
def dice_coefficient(pred:torch.Tensor, target:torch.Tensor, num_classes:int, ignore_index:int):
    ...
    pred = pred * torch.stack([mask]*2, dim=1)
    target = target*mask
    ...
    if num_classes == 1:
        ...
        target = target.type(pred.type())
        target = torch.eye(num_classes, device=pred.device)[target.long()].to(pred.device)
        target = target.permute(0, 2, 1)
        pred = F.softmax(pred, dim=1)

    inter = torch.sum(pred*target, dim=(2))
    sum_sets = torch.sum(pred+target, dim=(2))
    dice_coefficient = (2*inter / (sum_sets+1e-6)).mean(dim=0)
    return dice_coefficient
        
        
def dice_loss(pred:torch.Tensor, target:torch.Tensor, num_classes:int=2, weight:torch.Tensor=None, ignore_index:int=-100):
...
class PerceptualLoss(nn.Module):
    def __init__(self, layer_weights=None):
        ...
        vgg = models.vgg16(pretrained=True).features.eval().to(device='cuda')
    ...
    def forward(self, input_img, target_img):
        ...
        for i, layer in enumerate(self.vgg_layers):
            x = layer(x)
            y = layer(y)
            if i in [3, 8, 15]:  # conv1_2, conv2_2, conv3_3
                loss += self.layer_weights[layer_idx] * self.criterion(x, y)
                layer_idx += 1
